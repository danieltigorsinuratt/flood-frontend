...
export default function SensorDataIndexPage() {
  ...
  const load = useCallback(async (params = {}) => {
    setLoading(true);
    try {
      const { data } = await axios.get(route('monitoring.sensor-data.index'), {
        params: { ...filters, ...params },
      });
      setSensorData(data.sensorData ?? { data: [], links: [] });
      setDevices(data.devices ?? []);
      if (data.filters) setFilters(data.filters);
    } catch {
      setSensorData({ data: [], links: [] });
    } finally {
      setLoading(false);
    }
  }, [filters]);
  ...
  const removeRow = async (id) => {
    ...
    try {
      await axios.delete(route('monitoring.sensor-data.destroy', { id }));
      await load(filters);
    } catch {
      /* abaikan */
    }
  ...
  return (
    <AuthenticatedLayout title="Data sensor" header={<h2 className="text-xl font-semibold text-white">Data sensor</h2>}>
      <div className="py-8">
          ...
          >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setField('alert_level', e.target.value)}
              >
                <option value="">Semua</option>
              ...
              <table className="min-w-full divide-y divide-slate-700 text-sm">
                <thead className="bg-slate-800/80">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-slate-200">Hujan</th>
                  ...
                  {sensorData.data.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-800/50">
                      ...
                      <td className="px-4 py-3 text-white">
                        {row.relay_on == null ? '—' : row.relay_on ? 'ON' : 'OFF'}
                      </td>
                      <td className="px-4 py-3">
                        ...
                        >
                          {row.alert_level === 'danger' ? 'AWAS' : row.alert_level === 'warning' ? 'SIAGA' : 'NORMAL'}
                        </span>
                      </td>
