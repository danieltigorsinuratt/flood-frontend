...
export default function DevicesEditPage({ deviceId }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await axios.get(route('monitoring.devices.update', { id: deviceId }));
      setData({
  ...
  const submit = async (e) => {
    ...
    setErrors({});
    try {
      await axios.patch(route('monitoring.devices.update', { id: deviceId }), data);
      router.push(route('monitoring.devices.index'));
    } catch (err) {
  ...
  const remove = async () => {
    ...
    try {
      await axios.delete(route('monitoring.devices.destroy', { id: deviceId }));
      router.push(route('monitoring.devices.index'));
    } catch {
      /* abaikan */
    }
  ...
  if (loading) {
    return (
      <AuthenticatedLayout title="Edit perangkat">
        <p className="p-8 text-sm text-slate-400">Memuat…</p>
      </AuthenticatedLayout>
    );
  }
    ...
    <AuthenticatedLayout
      ...
      header={
        <div className="flex flex-col gap-1">
          ...
          <Link href={route('monitoring.devices.index')} className="text-sm text-amber-300 hover:text-amber-200">
            ← Kembali ke daftar
          </Link>
        </div>
      }
    >
      <div className="py-8">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <form onSubmit={submit} className="space-y-6 rounded-lg border border-slate-700 bg-slate-900 p-6 shadow-sm">
            <div>
              ...
              <TextInput
                ...
                required
              />
              <InputError message={errors.device_id?.[0]} className="mt-2" />
            </div>
            <div>
            ...
            <div>
              ...
              <TextInput
                ...
                required
              />
              <InputError message={errors.location?.[0]} className="mt-2" />
            </div>
            <div>
