...
export default function Index() {
    ...
    const load = async () => {
        const { data: body } = await axios.get(route('monitoring.commands.list'));
        setCommands(body.commands ?? { data: [] });
    ...
    const markDone = async (cmdId) => {
        await axios.post(route('monitoring.commands.executed', { id: cmdId }));
        await load();
    };

    const remove = async (cmdId) => {
        if (!confirm('Hapus command ini?')) return;
        await axios.delete(route('monitoring.commands.destroy', { id: cmdId }));
        await load();
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-white">Command</h2>}>
            <div className="py-8">
