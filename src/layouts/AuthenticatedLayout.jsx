...
export default function AuthenticatedLayout({
  ...
  const { user, isAdmin, flash, logout, loading } = useAuth();
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      let hostOk = false;
      try {
        const res = await fetch(`${origin}/up`, {
          method: 'GET',
      ...
      if (!cancelled) setApiIndicator(hostOk && live);
    };
    ...
    const loadFromFirmware = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data?.origin) { setApiDraft(data.origin); return; }
        }
      } catch { /* abaikan */ }
      if (!cancelled) setApiDraft(iotApiBaseUrl || '');
    };
  ...
  const hasNavbarExtras = (title != null && title !== '') || navbarTrailing != null;
  const sidebarOpenMobile = mobileMenuOpen;
  const sidebarOpenDesktop = lgUp && !desktopSidebarCollapsed;

  const sidebarAnimate = lgUp
    ? { width: desktopSidebarCollapsed ? 0 : 256, opacity: desktopSidebarCollapsed ? 0 : 1, x: 0 }
    : { x: mobileMenuOpen ? 0 : -304, opacity: 1 };

  const sidebarTransition = reduceMotion
    ? { duration: 0.15 }
    : lgUp
      ? {
          width: { type: 'tween', duration: 0.62, ease: easeOutSoft },
          opacity: { type: 'tween', duration: 0.38, ease: easeOutExpo },
        }
      : {
          x: sidebarOpenMobile
            ? { type: 'spring', stiffness: 165, damping: 24, mass: 0.95, restDelta: 0.35, restSpeed: 0.4 }
            : { type: 'spring', stiffness: 420, damping: 40, mass: 0.72 },
        };
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="flex min-h-screen">
        <AnimatePresence>
          {sidebarOpenMobile ? (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              transition={reduceMotion ? { duration: 0.1 } : { duration: 0.42, ease: easeOutExpo }}
        ...
        <motion.aside
          ...
          animate={{
            ...sidebarAnimate,
            pointerEvents:
              lgUp && desktopSidebarCollapsed ? 'none'
                : !lgUp && !mobileMenuOpen ? 'none'
                  : 'auto',
          }}
        ...
        >
          <div className="flex h-14 shrink-0 items-center justify-between gap-2 border-b border-slate-700 px-3">
            <Link href="/" className="flex min-w-0 flex-1 items-center gap-2" onClick={closeMobile}>
            ...
            <button
              type="button"
              onClick={() => { setMobileMenuOpen(false); setDesktopSidebarCollapsed(true); }}
          ...
          <nav className="flex min-h-0 flex-1 flex-col space-y-1 overflow-y-auto p-3">
            <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Utama</p>
            <SidebarNavLink href="/dashboard" active={path === '/dashboard'} onNavigate={closeMobile}>
              <span>Ringkasan</span>
            </SidebarNavLink>
            <SidebarNavLink href="/dashboard/riwayat" active={path === '/dashboard/riwayat'} onNavigate={closeMobile}>
              <span>Riwayat</span>
            </SidebarNavLink>

            <p className="mb-2 mt-6 px-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Integrasi</p>
            <button
              type="button"
              onClick={() => { setApiModalOpen(true); closeMobile(); }}
              className={
            ...
            >
              ...
              <span
                className={
                  ...
                  (apiIndicator === true ? 'bg-emerald-500' : apiIndicator === false ? 'bg-red-500' : 'bg-slate-300')
                }
                title={
                  apiIndicator === true ? 'Host API OK · telemetri hidup'
                    : apiIndicator === false ? 'Merah: tidak ada telemetri baru atau host API tidak terjangkau'
                      : 'Memeriksa…'
                }
                aria-label={
                  apiIndicator === true ? 'Host API OK dan telemetri hidup'
                    : apiIndicator === false ? 'Telemetri tidak hidup atau API tidak terjangkau'
                      : 'Memeriksa koneksi'
                }
              />
            </button>
            <SidebarNavLink href="/dashboard/download" active={path === '/dashboard/download'} onNavigate={closeMobile}>
              <span>Download</span>
            </SidebarNavLink>
            <SidebarNavLink href="/dashboard/kalender" active={path === '/dashboard/kalender'} onNavigate={closeMobile}>
              <span>Kalender</span>
            </SidebarNavLink>

            {isAdmin ? (
              <>
                <p className="mb-2 mt-6 px-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Monitoring</p>
                <SidebarNavLink href="/monitoring/devices" active={path.startsWith('/monitoring/devices')} onNavigate={closeMobile}>
                  <span>Perangkat</span>
                </SidebarNavLink>
                <SidebarNavLink href="/monitoring/sensor-data" active={path.startsWith('/monitoring/sensor-data')} onNavigate={closeMobile}>
                  <span>Data sensor</span>
                </SidebarNavLink>
                <SidebarNavLink href="/monitoring/commands" active={path.startsWith('/monitoring/commands')} onNavigate={closeMobile}>
                  <span>Command</span>
                </SidebarNavLink>
              </>
            ) : null}
          ...
          >
            ...
            {hasNavbarExtras ? (
              <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4">
                ...
                {navbarTrailing ? (
                  <div className="flex shrink-0 items-center gap-2">{navbarTrailing}</div>
                ) : null}
              </div>
            ) : (
              <div className="min-w-0 flex-1" />
              ...
              <Dropdown>
                <Dropdown.Trigger>
                  ...
                  >
                    ...
                    <svg className="h-4 w-4 shrink-0 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                ...
                <Dropdown.Content align="right" width="48">
                  ...
                  <Dropdown.Link href="#" as="button" onClick={logout}>Log Out</Dropdown.Link>
                </Dropdown.Content>
              </Dropdown>
            </div>
          </motion.header>
      ...
      {apiModalOpen ? (
        ...
        >
          ...
          >
            <h3 id="iot-api-host-title" className="text-lg font-semibold text-white">
            ...
            <p className="mt-1 text-[11px] leading-snug text-slate-400">
              Masukkan URL asal backend Laravel kamu. Contoh:{' '}
              <code className="rounded bg-slate-800 px-1 text-[10px] text-slate-200">http://127.0.0.1:8000</code>
            </p>
            <input
              type="url"
              inputMode="url"
              ...
              value={apiDraft}
              onChange={(e) => { setApiDraft(e.target.value); setApiFormError(null); }}
            />
            {apiFormError ? <p className="mt-2 text-sm text-red-400">{apiFormError}</p> : null}
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              ...
              <button
                type="button"
                className="rounded-md border border-slate-600 px-3 py-1.5 text-sm text-white hover:bg-slate-800"
                onClick={() => { clearIotApiBase(); setApiDraft(''); setApiFormError(null); setApiModalOpen(false); }}
              >
                Pakai situs ini
              </button>
              <button
                ...
                onClick={async () => {
                  ...
                  if (!value) {
                    try {
                      ...
                      if (res.ok) {
                        const data = await res.json();
                        if (data?.origin) { value = data.origin; setApiDraft(data.origin); }
                      }
                    } catch { /* abaikan */ }
                  }
                  if (!value) {
                    setApiFormError('Isi URL atau pastikan backend Laravel berjalan.');
                    return;
                  }
                  const r = saveIotApiBase(value);
                  if (!r.ok) { setApiFormError(r.error ?? 'Gagal menyimpan.'); return; }
                  setApiFormError(null);
